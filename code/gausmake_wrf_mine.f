      module mod_inc
         integer ilon,jlat,alon,alat,jlat2,day0
         parameter (alon=500)
         parameter (alat=500)
...
c----------------------------------------------------------------------
      ...
      integer ny,nm,nd,nh,hint,yr1,yr2,mm1,mm2,yy,yd,dlen,dd1,dd2
      integer i,j,k,iunit,icode,inet,inet2,inet3,latv,lonv
      integer ncopn,ncvid,utopen,Ddir
...
c------------------------------------------------------------
      read(3,10)vtype,rtype,vunits,
     & dystrt,hrstrt,dystop,hrstop,hint,ilon,jlat,Ddir,dir,day0
      close(3)
10    format (//a8/a5/a8/i8/i2/i8/i2/i2/i3/i3/i2/a20/i8)

      vname='slp0'
      xname='lon0'
      yname='lat0'

      name='WRF_d01_slp_regrid.nc'
      write(path,'(a,i2.2,a)')'/srv/ccrc/data',Ddir
     &,'/z3478332/WRF/output/ERAI_R2_nudging_'      
...
cc Call the first year's netcdf file. Extract lat & lon      
      ...
      write(ifile,'(a,a,a,a)')trim(path)
     &,trim(dir),'/out/',trim(name)
      print *, ifile
      ...
      if (yd.eq.0) then   
        if(mm1.eq.mm2) then
          do j=dd1,dd2
              do k=1,hint
                 nh=(24/hint)*(k-1)
                 call write_ausm(yr1,mm1,j,nh,inet,2)
              enddo 
          enddo
        else
          do i=mm1,mm2
...
CC Declare all the internal variables
      ...
      integer inet,iunit,ivar,yy0,mm0,dd0
      integer*2 miss
      integer start(3)
      integer count(3)
...
CC  Extract and unpack the needed timestep of data      
      ...
      yy0= floor(real(day0)/10000.0)
      mm0= mod(floor(real(day0)/100.0),100)
      dd0= mod(day0,100)
      
      call xhour(ny,nm,nd,nh,it)
      call xhour(yy0,mm0,dd0,0,ot)
      tt = (it-ot)/6+1
