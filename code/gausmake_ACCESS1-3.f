...
c------------------------------------------------------------
      ...
      write(ifile1,'(a,a,a,a,a,a)')trim(path)
     &,'historical/fx/orog/',trim(name),'/r0i0p0/orog_fx_'
     &,trim(name),'_historical_r0i0p0.nc'
...
cc Going to have to reload the file each time
cc Because the stupid files start at 6Z
...
c For 00Z on 1/1/1950 and 1/1/2006
      ...
      if(rcp.eq."historical") then
        dd=ny/10
c If date is YYY0010100 nd=nd-1
        if(mdh.eq.10100.and.ny.eq.dd*10) then
          dd=dd-1
        endif
...
c Need to use stupid decades 2006-2016 etc
        dd=(ny-6)/10
c If date is YYY0010100 nd=nd-1
        if(mdh.eq.10100.and.ny-6.eq.(dd*10)) then
          dd=dd-1
        endif
     ...
     &,trim(name),'/',trim(mem),'/psl_6hrPlev_',trim(name)
     &,"_",trim(rcp),'_',trim(mem),"_",y1,"010106-",y2,"010100.nc"
c      print *, ifile
      inet=ncopn(ifile,0,icode)

 2001 format (a,a,a,a,a1,a,a,a,a1,a,a1,a,a1,i4,a,i4,a)

      call xhour(ny,nm,nd,nh,it)
      call xhour(y1,1,1,6,ot)
      tt = (it-ot)/6+1
