      module mod_inc
         integer ilon,jlat,alon,alat,jlat2,day0,lonS,latS
         parameter (alon=500)
         parameter (alat=500)
...
c------------------------------------------------------------
     ...
     & lonS,latS,path,day0
      close(3)
10    format (//a8/a5/a8/a10/i8/i2/i8/i2/i2/i3/i3/i3/i3/a100/i8)

      vname='slp0'
      xname='lon0'
      yname='lat0'

      name='WRF_d02_slp_regrid.nc'
...
cc Call the first year's netcdf file. Extract lat & lon      
     ...
     &,trim(name)
      ...
      call ncvgt(inet,latv,latS,jlat,lat1,icode)
      latitude=real(lat1)
      lonv=ncvid(inet,xname,icode)
      call ncvgt(inet,lonv,lonS,ilon,lon1,icode) 
      longitude=real(lon1)
...
c      print *, latitude(1), longitude(1), x(1,1)
c      print *, latitude(jlat), longitude(ilon), x(ilon,jlat)
