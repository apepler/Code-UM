...
c----------------------------------------------------------------------
      ...
      instfile = 'ingausmake'
      ofile    = 'tape11'
      path = '/srv/ccrc/data34/z3478332/MERRA/'
...
c------------------------------------------------------------
     ...
     & dystrt,hrstrt,dystop,hrstop,hint,ilon,jlat,sres
      close(3)
...
c  Assumes a file of format 'name',YYYY,'_regrid.nc'. 
      name='MERRA_slp_'
...
cc Call the first year's netcdf file. Extract lat & lon      
      ...
      write(ifile,'(a,a,i4,a)')trim(path)
     &,trim(name),yr1,'_regrid.nc'
...
cc loop on months/day/hours to extract & write code       
      if (yd.eq.0) then   
         ...
         write(ifile2,'(a,a,i4,a)')trim(path)
     &   ,trim(name),yr2,'_regrid.nc'
...
cc Loops twice - once for each year
     ...
     &   ,trim(name),yr2,'_regrid.nc'
         inet2=ncopn(ifile2,0,icode)
         write(ifile3,'(a,a,i4,a)')trim(path)
     &   ,trim(name),yy,'_regrid.nc'
