      module mod_inc
         integer ilon,jlat,alon,alat,jlat2,ilev,level
         parameter (alon=600)
         parameter (alat=600)
         parameter (ilev=37)
...
c z_6hrs_ERAI_historical_an-pl_YYMMDD1_YYMMDD2.nc 
      name='z_6hrs_ERAI_historical_an-pl_'
...
c------------------------------------------------------------
     ...
     & dystrt,hrstrt,dystop,hrstop,hint,ilon,jlat,level
      close(3)
...
cc Call a random month's netcdf file to xtract lat & lon      
      write(ifile,'(a,a)')trim(path),'z_6hrs_ERAI_' //
     + 'historical_an-pl_20070101_20070131.nc'
...
cc Then do loop on days,hours to save ausmap
     ...
     &      ,trim(name),day1,'_',day2,'.nc'
            inet=ncopn(ifile,0,icode)   
            do j=1,dlen
               call write_ausm(yr1,i,j,inet,2)
            enddo
         enddo  
...
cc Loops twice - once for each year
     ...
     &      ,trim(name),day1,'_',day2,'.nc'
            inet=ncopn(ifile,0,icode)            
            do j=1,dlen
              call write_ausm(yr2,i,j,inet,2)
            enddo
         enddo  
      endif                    
...
CC   Subroutine to load, format & write a date
      ...
      subroutine write_ausm(ny,nm,nd,inet,iunit)
...
CC Declare all the internal variables
      ...
      real work(ilon,jlat)
      integer it,ot,tt,tlen,tvar,icode,ny,nm,nd,nh,ll
...
CC  Assumes the netcdf file is of a single MONTH with 6-hourly data
      ...
      call xhour(ny,nm,nd,0,it)
      call xhour(ny,nm,1,0,ot)
      tt = (it-ot)/6+1

cc Get the right level.
      ivar=ncvid(inet,'lev',icode)
      call ncvgt(inet,ivar,1,ilev,lev,icode)
      lev=lev/100
...
c      print*, x(1,1), x(ilon,jlat), minval(x), maxval(x)
